...
int useGPU::ImageToGrayGpu(unsigned char* imageRGBA, int width, int height) {
	// Cuda Event Timers
	float GPUTime = 0.0f;
	cudaEvent_t start_01;
	cudaEvent_t stop_01;
	cudaEventCreate(&start_01);
	cudaEventCreate(&stop_01);
	...
	// Start Cuda Timer
	cudaEventRecord(start_01);
	ConvertImageToGrayGpu << <gridSize, blockSize >> > (ptrImageDataGpu);
	// Stop Cuda Timer
	cudaEventRecord(stop_01);
	//Compute Cuda Timer
	cudaEventElapsedTime(&GPUTime, start_01, stop_01);
